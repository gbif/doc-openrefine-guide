...
En las primeras columnas del proyecto encontrará de manera intercalada una columna con el valor taxonómico original, un valor sugerido de acuerdo al árbol taxonómico de GBIF y el indicador de validación indicando si los valores son iguales (1) o difieren (0) como se muestra en la (<<img-fig-58,Figura 58>>).
...
WARNING: Antes de ejecutar la rutina remplace la palabra _demo_  en la expresión `_username=demo_` por el nombre de usuario de su cuenta de GeoNames, por ejemplo `_username=rortizgt_`. Si ejecuta la rutina sin hacer este cambio utilizará la opción de prueba (_demo_ ) la cual tiene el limite de  20.000 consultas *diarias mundiales*, por lo que al corre la rutina de este modo corre el riesgo de que el servicio no esté disponible y por lo tanto no obtendrá resultados.


La rutina obtiene la elevación asociada a las coordenadas documentadas en los elementos [source]`"decimalLatitude"` y [source]`"decimalLongitude"` a partir del llamado al  del API de link: http://www.geonames.org/export/web-services.html[GeoNames] que utiliza el modelo de elevación STMR-1. La elevación es obtenida con una resolución de 30 metros por pixel, la rutina compara esta elevación contra el elemento [source]`"minimumElevationInMeters"` en el archivo base contra, generando los indicadores de validación.

IMPORTANT: Se recomienda hacer la consulta sobre valores únicos de coordenadas y no sobre el total de los registros ya que el límite del servicio de GeoNames es de 2.000 registros diarios por usuario.

*Resultados:*

En las primeras columnas del proyecto encontrará de manera intercalada una columna con el valor de elevación original, la elevación sugerida por GeoNames a partir de las coordenadas y el indicador de validación indicando si los valores son iguales (1) o difieren (0) como se muestra en la (<<img-fig-60,Figura 60>>).
