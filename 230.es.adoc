...
Ubique el campo [source]`"specificEpithet"` y cree una faceta de texto (haga click en "la &#9660; azul > Facetas > Faceta de texto"). Luego vaya nuevamente a menu:la{sp}&#9660;{sp}azul[] y cree un filtro de texto (“Filtro de texto”). Sobre el menú de la izquierda se abrirá una ventana como la que se muestra en la <<img-fig-27,Figura 27>>.
...
En el menú de la izquierda, dentro de la faceta se mostrará el valor que usted buscó, y en la pantalla principal se mostrarán los registros asociados que tienen dicho valor en el campo [source]`"specificEpithet"` (<<img-fig-28,Figura 28>>).
...
Abra una faceta de texto y un filtro para el campo [source]`"scientificName"`. En el filtro, busque el valor “sp.”. Verá entonces varios valores para ese campo que incluyen “sp.”, como se muestra en la <<img-fig-29,Figura 29>>.
...
Haga click sobre menu:la{sp}&#9660;{sp}azul{sp}en{sp}{scientificName}[Editar celdas > Transformar…] (<<img-fig-30,Figura 30>>).

[#img-fig-30]
.Figura 30
image::img/es.figure-30.jpg[Figura 30,align=center]

Se abrirá entonces una ventana como la mostrada en la (<<img-fig-31,Figura 31>>). En el cuadro de texto, pegue la siguiente expresión:
[source,javascript]
----
value.replace(" sp.", "")
----
...
En la <<img-fig-31,Figura 31>> puede observar cómo se vería el resultado del cambio en la pestaña “Vista previa”. 
...
A modo de ejemplo, buscaremos valores en el campo [source]`"genus"` que comiencen con minúscula. Para ello, abra una faceta y un filtro de texto para el campo [source]`"genus"`. En el filtro coloque la siguiente expresión en el cuadro de texto: [source,regex]`^[a-z]`, y seleccione las opciones “Distingue mayúsculas y minúsculas” y “regex” (<<img-fig-32,Figura 32a>>). Con dicha expresión se pueden buscar los valores en los que la primera letra es minúscula.

[#img-fig-32]
.Figura 32
image::img/es.figure-32.jpg[Figura 32,align=center]

Siguiendo estos pasos, debería poder ver dos valores (<<img-fig-32,Figura 32b>>). Corrija estos valores filtrados, dado que el género debe comenzar con mayúscula.
