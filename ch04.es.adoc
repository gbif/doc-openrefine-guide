...
* [source]`"AAAA"`: año, con cuatro dígitos.
* [source]`"MM"`: mes, con dos dígitos. E.g.: mayo sería 05.
* [source]`"DD"`: día, con dos dígitos. E.g.: segundo día de un mes sería 02.
* [source]`"T"`: indica que lo que viene a continuación es la hora.
* [source]`"HH"`: horas, con dos dígitos, en formato de 24 hs.
* [source]`"mm"`: minutos, con dos dígitos.
* [source]`"X"`: indica la zona horaria. La zona horaria se determina tomando como base UTC (Coordinated Universal Time). Si uno está justo sobre la zona horaria UTC, X se reemplaza por “Z”. Si uno está en otra zona horaria, debe reemplazarse X por la diferencia horaria correspondiente.
